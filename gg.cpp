...
void generate_conc(int dim, MMSP::grid<2, unsigned long> *grid, const char* filename)
{
	const int L = 32; 
	...
	if (dim == 2) {
		GRID2DArr initGrid(3,0,L,0,L); //0 is concentration, 1 is phase, 2 is state.
		for (int d = 0; d < dim; d++)
		...
		for (int i = 0; i < nodes(initGrid); i++) {
			...
			if (x[0] <= L/2) {
				initGrid(i)[1] = 1.;
				initGrid(i)[0] = 0.7	; // Initial concentration
			}
			else {
				initGrid(i)[1] = 0.;
				initGrid(i)[0] = 0.3; // Initial concentration
			}
    vector<int> r(dim,0);
