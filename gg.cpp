...
void generate(int dim, const char* filename)
{
	...
	if (dim == 2) {
		...
		unsigned long timer = tessellate<2, unsigned long>(grid, number_of_fields, 16	);
...
template <int dim, typename T> void update(grid<dim, vector<T> >& oldGrid, int steps, int fileIncrement)
{
	int id = 0;
	int np = 1;
	...
	for (int step = 0; step < steps; step++) {
		...
		if (step % fileIncrement == 0)
		{
			...
			pstr << "p_grid" << step;
			gstr << "g_grid" << step;
			grid<dim, double > c_grid(1, 0, L, 0, L);
			grid<dim, double > p_grid(1, 0, L, 0, L);
			grid<dim, double > g_grid(1, 0, L, 0, L);
			for (int i = 0; i < nodes(oldGrid); i++) {
				c_grid(i) = oldGrid(i)[0];
				p_grid(i) = oldGrid(i)[1];
				g_grid(i) = oldGrid(i)[2];
			}

			MMSP::output(c_grid, cstr.str().c_str());
			MMSP::output(p_grid, pstr.str().c_str());
			MMSP::output(g_grid, gstr.str().c_str());
		}
	}
